...
class TenantsClientTest(TestCase):
    ...
    def test_list_tenants(self):
        ...
        assert HTTPretty.last_request.headers['Content-Type'] == 'application/{0}'.format(self.serialize_format)
        assert 'application/{0}'.format(self.serialize_format) == HTTPretty.last_request.headers['Accept']
    ...
    def test_get_users_roles_for_tenant(self):
        url = "{0}/v2.0/tenants/{1}/users/{2}/roles".format(self.url, self.tenant_id, self.user_id)
        HTTPretty.register_uri(HTTPretty.GET, url, body=self._build_list_of_roles_for_tenant())

        actual_response = self.tenant_api_client.get_users_roles_on_tenant(tenant_id=self.tenant_id,
