...
class ExtensionsTest(TestCase):
  def setUp(self):
    ...
    self.dict_for_value = {
                          ...
                          'name': self.name,
                          'links': [{"href": self.href, "type": self.type, "rel": self.rel}],
    ...
    self.serialized_str_for_extensions = '{"extensions": {"extensions": [{"values": []}]}}'
    self.dict_for_extensions = {'extensions': [{'values':self.values}]}
  ...
  def test_json_to_obj(self):
    assert Extensions._json_to_obj(self.serialized_str_for_extensions) == self.extensions
